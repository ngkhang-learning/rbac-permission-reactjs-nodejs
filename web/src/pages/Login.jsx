...
function Login() {
  ...
  const submitLogIn = async (data) => {
    ...
    // Save tokens and userInfo into Local Storage
    localStorage.setItem('accessToken', res.data.tokens.accessToken);
    localStorage.setItem('refreshToken', res.data.tokens.refreshToken);
    localStorage.setItem('userInfo', JSON.stringify(res.data.userInfo));

    // toast.success(res.data?.message);
    // Direct to Dashboard page when login success
    navigate('/dashboard');
  };
    ...
    <Box
      ...
      <form onSubmit={handleSubmit(submitLogIn)}>
        <Zoom in={true} style={{ transitionDelay: '200ms' }}>
          <MuiCard
            sx={{ minWidth: 380, maxWidth: 380, marginTop: '6em', p: '0.5em 0', borderRadius: 2 }}>
            ...
            <Box
              ...
              <Box>
                <Typography>Hint: khangn@gmail.com</Typography>
                <Typography>Pass: khangn@123</Typography>
              </Box>
            </Box>
